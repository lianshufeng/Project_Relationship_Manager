...
export type PersonActivitySourceType = 'deviceType' | 'product';
export interface PersonActivityItem { id: string; activityDate: string; sourceType: PersonActivitySourceType; sourceId: string; sourceNameSnapshot: string; activityValue: number; revision: number; createdAt?: string; updatedAt?: string }
export interface PersonActivitySummary { date: string; total: number; deviceTypes: Array<{ sourceId: string; activityValue: number }>; products: Array<{ sourceId: string; activityValue: number }> }
export interface PersonActivityRange { from: string; to: string; items: PersonActivityItem[] }
export interface Person { id: string; name: string; phone?: string; avatar?: string; icon: string; positionIds: string[]; description?: string; todayActivity?: PersonActivitySummary }
...
export interface ProjectRelationshipData { schemaVersion: '1.0'; dataRevision: 18; project: Project; teams: Team[]; positions: Position[]; persons: Person[]; products: ProductModule[]; deviceTypes: DeviceType[]; devices: DeviceInstance[]; areas: Area[]; relations: Relation[]; personActivities?: PersonActivityItem[]; feedbackSummaries: Record<string, { total: number; unanswered: number }>; settings: GraphSettings }
